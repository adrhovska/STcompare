...
from STalign import (
    STalign,
)  # because the name of the module is the same as of the package
...
def parse_args():
    ...
    parser.add_argument(
        "--scale2", required=True, help="Reference scalefactors_json.json"
    )
    parser.add_argument(
        "--points1", required=True, help="Source manual landmarks CSV with columns y,x"
    )
    parser.add_argument(
        "--points2",
        required=True,
        help="Reference manual landmarks CSV with columns y,x",
    )
    parser.add_argument(
        "--project_dir",
        default=Path.cwd(),
    ...
    parser.add_argument(
        "--image2", default=None, help="Reference tissue_hires_image.png"
    )
    parser.add_argument(
        "--alignment_method",
        default="`stalign",
        choices=["affine", "stalign"],
        help="Alignment method: affine or stalign.",
    )
    ...
    parser.add_argument(
        ...
        default=100,
        type=int,
        help="Iteration when nonlinear deformation starts.",
    )
    ...
    parser.add_argument(
        "--skip_stalign_qc", action="store_true", help="Skip extra STalign QC plots."
    )
    parser.add_argument(
        "--stalign_grid_levels",
        default=20,
        type=int,
        help="Number of contour levels for the deformation-grid QC plot.",
    )
...
"""adding x and y coordinates in hires coordinate system
...
def add_xy_coordinates(df, scalefactors):
    ...
    df["x"], df["y"] = (
        df["pxl_col_in_fullres"].astype(float) * factor,
        df["pxl_row_in_fullres"].astype(float) * factor,
    )
    return df


# filtering spots to only those in tissue
def filter_spots(df):
...
def validate_spots(df, sample_name):
    ...
    print(
        f"{sample_name}: {df.shape[0]} in-tissue spots"
    )  #  is for rows who are barcodes and thus spots


## Affine transformation

"""reading manual landmark points from CSVs saved from LandmarkPicker.py
...
"""fitting affine transformation from source to reference landmarks
...
def fit_affine(points1_yx, points2_yx):
    if points1_yx.shape != points2_yx.shape:
        raise ValueError(
            "Source and reference landmark arrays must have the same shape."
        )
...
# applying affine transformation to x and y coordinates (landmarks and source spots)
...
# printing affine matrix and residuals of the fit
def print_affine_summary(affine, residuals):
    ...
    singular_values = np.linalg.svd(
        linear_part, compute_uv=False
    )  # stretching, should remain similar on both sides
    determinant = np.linalg.det(
        linear_part
    )  # should be positive, if negative then the image is flipped
    print(f"Affine scale singular values: {singular_values}")
    print(f"Affine determinant: {determinant:.2f}")


# shared: builds the standard aligned-spots output table from raw aligned_x/y arrays
def build_aligned_spot_table(src, aligned_x, aligned_y):
    src_out = src.copy()
    src_out["original_x"], src_out["original_y"] = src_out["x"], src_out["y"]
    src_out["aligned_x"], src_out["aligned_y"] = aligned_x, aligned_y
    src_out["x"], src_out["y"] = src_out["aligned_x"], src_out["aligned_y"]
    preferred_cols = [
        "barcode",
        "x",
        "y",
    ...
    return src_out[preferred_cols]


# applying alignment to source spot table (affine)
def align_source_spots(src, affine):
    aligned_x, aligned_y = apply_affine(
        src["x"].to_numpy(), src["y"].to_numpy(), affine
    )
    return build_aligned_spot_table(src, aligned_x, aligned_y)


# transforming source spots to reference coordinate system using STalign output
def align_source_spots_stalign(src, stalign_out):
    ...
    transformed_yx = STalign.transform_points_source_to_target(
        stalign_out["xv"], stalign_out["v"], stalign_out["A"], source_points_yx
    )
    transformed_yx = to_numpy(transformed_yx)
    aligned_y, aligned_x = transformed_yx[:, 0], transformed_yx[:, 1]
    return build_aligned_spot_table(src, aligned_x, aligned_y)
...
def make_landmark_fit_plot(points1_yx, points2_yx, affine, outpath):
    source_x, source_y = points1_yx[:, 1], points1_yx[:, 0]
    reference_x, reference_y = points2_yx[:, 1], points2_yx[:, 0]
...
# saving affine transform and landmark information as a compressed .npz file
# contains affine matrix, landmark residuals, and source/reference points
...
def prepare_stalign_image(img):
    ...
    y, x = np.arange(img.shape[1]) * 1.0, np.arange(img.shape[2]) * 1.0
    return [y, x], img


"""checking image input for STalign
...
def run_stalign_registration(args, points1, points2):
    if args.image1 is None or args.image2 is None:
        raise ValueError(
            "--image1 and --image2 are required for --alignment_method stalign."
        )
    device = "cuda:0" if torch.cuda.is_available() else "cpu"

    ## 1. Affine transformation
    # reading source and reference H&E images
    img1, img2 = read_he_image(args.image1), read_he_image(args.image2)
    ...
    ## 2. Non-linear LDDMM transformation
    ...
    out = STalign.LDDMM(
        xI, I, xJ, J, **params
    )  # here is the issue (print the args - might be empty)
...
# adding spot-level STalign QC metrics for both affine and LDDMM transformation
    ...
    if "WM" in stalign_out:
        try:
            ...
            spot_yx_tensor = torch.as_tensor(
                spot_yx, dtype=torch.float64, device=wm.device
            )
...
def main():
    ...
    src_raw, src_sf = read_spots(args.pos1), read_scalefactors(args.scale1)

    # reading reference sample
    tgt_raw, tgt_sf = read_spots(args.pos2), read_scalefactors(args.scale2)

    # adding coordinates
    src, tgt = add_xy_coordinates(src_raw, src_sf), add_xy_coordinates(tgt_raw, tgt_sf)

    # filtering to in-tissue spots
    src, tgt = filter_spots(src), filter_spots(tgt)
    ...
    # 2. STalign version
        ...
        src_out = stalign_qc(src_out, stalign_out, stalign_data)

        # landmark fit plot
        make_stalign_landmark_fit_plot(
            points1, points2, stalign_out, paths["landmark_plot"]
        )

        # STalign QC plots
        if not args.skip_stalign_qc:
            make_stalign_input_qc_plot(
                stalign_data, points1, points2, paths["stalign_input_qc_plot"]
            )
            make_stalign_initial_affine_qc_plot(
                stalign_data, points1, points2, paths["stalign_initial_affine_qc_plot"]
            )
            make_stalign_deformed_image_qc_plot(
                stalign_data, stalign_out, paths["stalign_deformed_image_qc_plot"]
            )
            make_stalign_deformation_grid_qc_plot(
                stalign_data,
                stalign_out,
            ...
            make_stalign_displacement_histogram(
                src_out, paths["stalign_displacement_hist_plot"]
            )
            make_stalign_wm_qc_plots(
                src_out, paths["stalign_wm_spot_plot"], paths["stalign_wm_hist_plot"]
            )
            make_stalign_lddmm_diagnostic_plot(
                stalign_data, stalign_out, paths["stalign_lddmm_diagnostic_plot"]
            )
    ...
    make_qc_summary_panel(
        ...
        paths["qc_summary_plot"],
        title=f"QC summary: {args.sample_aligned} aligned to {args.sample_reference}",
    )
