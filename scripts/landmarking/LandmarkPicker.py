...
default_dir = (
    Path.cwd()
)  # default directory is the directory where the script is run but can be overriden
...
def parse_args():
    ...
    parser.add_argument(
        "--image2", required=True, help="Reference tissue_hires_image.png"
    )
    parser.add_argument("--sample_aligned", required=True, help="Source sample name")
    parser.add_argument(
        "--sample_reference", required=True, help="Reference sample name"
    )
    parser.add_argument(
        "--project_dir",
        default=default_dir,
        type=Path,
        help="Output project directory. Defaults to the current working directory.",
    )  # type = Path helps it from returning a string to the path being stored as an object
...
def main():
    ...
    args = parse_args()
    image1, image2 = Path(args.image1), Path(args.image2)
    ...
    outdir = (
        project_dir
        / "landmarks"
        / f"{args.sample_aligned}_paired_to_{args.sample_reference}"
    )
    outdir.mkdir(parents=True, exist_ok=True)

    # creating paths for the output CSVs files
    out1, out2 = (
        outdir / f"{args.sample_aligned}_points.csv",
        outdir / f"{args.sample_reference}_points.csv",
    )
    out_combined = (
        outdir
        / f"{args.sample_aligned}_and_{args.sample_reference}_landmark_pairs_combined.csv"
    )
    ...
    for i in range(n_points):
        y1, x1 = click_landmark(
            img1, f"{args.sample_aligned}/source: click landmark {i + 1}"
        )
        points1.append(
            {
                "landmark": i + 1,
        ...
        y2, x2 = click_landmark(
            img2, f"{args.sample_reference}/reference: click matching landmark {i + 1}"
        )
        points2.append(
            {
                "landmark": i + 1,
    ...
    df1, df2 = pd.DataFrame(points1), pd.DataFrame(points2)
