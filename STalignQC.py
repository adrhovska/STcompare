...
"""converting torch tensors (used internally by STalign) to numpy (used in table code and plots)
...
"""preparing images for matplotlib plotting 
    converts to numpy, makes channel-last, squeezes single-channel images and rescales on 0 to 1 range
    warns and returns a flat mid-gray image if the array has no dynamic range (all finite pixels equal)
"""
...
def image_for_plot(img):
    ...
    if not finite.any():
        print(
            "Warning: image_for_plot received an image with no finite values; returning as-is."
        )
        return arr
    ...
    if hi > lo:
        ...
        print(
            f"Warning: image_for_plot received a flat image (all finite values = {lo}); this often indicates a registration failure and now returns a mid-gray placeholder."
        )
        arr = np.full_like(arr, 0.5)
    return arr


# getting the extent of the image for plotting in matplotlib
...
# converting STalign image and axes to matplotlib format for plotting
...
# saving and closing for all QC figures
def save_and_close(fig, outpath, **kwargs):
    fig.tight_layout()
    fig.savefig(outpath, dpi=300, **kwargs)
    plt.close(fig)
...
def make_stalign_landmark_fit_plot(points1_yx, points2_yx, stalign_out, outpath):
    transformed_yx = STalign.transform_points_source_to_target(
        stalign_out["xv"], stalign_out["v"], stalign_out["A"], points1_yx
    )
    ...
    aligned_x, aligned_y = transformed_yx[:, 1], transformed_yx[:, 0]
    ...
    ax.legend()
    ax.set_title(f"STalign landmark fit / mean residual = {residuals.mean():.2f}px")
    save_and_close(fig, outpath)
    print(
...
def make_stalign_initial_affine_qc_plot(stalign_data, points1_yx, points2_yx, outpath):
    ...
    ax[0].scatter(
        transformed_landmarks[:, 1],
        transformed_landmarks[:, 0],
        s=35,
        label="source landmarks after affine",
    )
...
def make_stalign_deformed_image_qc_plot(stalign_data, stalign_out, outpath):
    ...
    phiI = STalign.transform_image_source_to_target(
        stalign_out["xv"], stalign_out["v"], stalign_out["A"], xI, I, xJ
    )
...
def make_stalign_deformation_grid_qc_plot(
    stalign_data, stalign_out, points1_yx, points2_yx, outpath, grid_levels=20
):
    xI, I, xJ, J = (
        stalign_data["xI"],
        stalign_data["I"],
    ...
    phii = STalign.build_transform(
        stalign_out["xv"], stalign_out["v"], stalign_out["A"], XJ=xJ, direction="b"
    )
    phiI = STalign.transform_image_source_to_target(
        stalign_out["xv"], stalign_out["v"], stalign_out["A"], xI, I, xJ
    )
    ...
    ax.scatter(
        ...
        transformed_landmarks[:, 0],
        s=45,
        label="source landmarks after STalign",
    )
...
def make_stalign_spots_on_target_qc_plot(src_out, tgt, stalign_data, outpath, title):
    xJ, J = stalign_data["xJ"], stalign_data["J"]
    ...
    ax.scatter(
        ...
        s=8,
        alpha=0.35,
        label="source spots after STalign",
    )
    ax.set_title(title)
    ax.set_aspect("equal")
    ax.invert_yaxis()
...
def make_stalign_displacement_histogram(src_out, outpath):
    ...
    fig, ax = plt.subplots()
    ax.hist(
        src_out["stalign_total_displacement"].dropna(),
        bins=40,
        alpha=0.55,
        label="total",
    )
    if "stalign_nonlinear_displacement" in src_out.columns:
        ax.hist(
            src_out["stalign_nonlinear_displacement"].dropna(),
            bins=40,
            alpha=0.55,
            label="nonlinear after affine",
        )
...
def make_qc_summary_panel(paths, alignment_method, outpath, title=None):
    qc_order = [
        ...
        (
            "QC5",
            "Deformed source vs reference",
            "stalign_deformed_image_qc_plot",
            ["stalign"],
        ),
        ("QC6", "Deformation grid", "stalign_deformation_grid_qc_plot", ["stalign"]),
        (
            "QC7",
            "Aligned spots on target",
            "stalign_spots_on_target_qc_plot",
            ["stalign"],
        ),
        (
            "QC8",
            "Spot displacement histogram",
            "stalign_displacement_hist_plot",
            ["stalign"],
        ),
